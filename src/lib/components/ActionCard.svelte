<script lang="ts">
  ...
  import { BookOpen, Clock, Star } from 'lucide-svelte';
...
<div class="card p-6 relative overflow-hidden">
  ...
  <div class="relative z-10">
    ...
    <div class="flex items-start space-x-3 mb-4">
      <BookOpen class="w-6 h-6 text-secondary-600 mt-1 flex-shrink-0" />
      <div>
    ...
    <div class="flex flex-wrap gap-2 mb-4">
      <span class="px-3 py-1 rounded-full text-xs font-medium {categoryColors[action.category]}">
        {action.category}
      </span>
      ...
      <div class="bg-secondary-50 rounded-lg p-4 mb-4">
        ...
        <ul class="space-y-1">
          {#each action.resources as resource}
            <li class="text-sm text-secondary-800">• {resource}</li>
          {/each}
        </ul>
      </div>
    {/if}
